-- Run this in the Supabase SQL editor.
-- 1. Replaces get_featured_program_ratings to add completion counts (completed_count, total_count).
-- 2. Adds get_program_engagement_trend for weekly workout volume per featured program.
--
-- Completion definition: employee logged at least one synced_workout matched
-- via planId UUID to an employer-assigned workout_plan.

-- ─── 1. Updated ratings + completion RPC ─────────────────────────────────────

CREATE OR REPLACE FUNCTION get_featured_program_ratings(p_practitioner_id UUID)
RETURNS TABLE(
  plan_name          TEXT,
  avg_effectiveness  NUMERIC,
  avg_enjoyment      NUMERIC,
  avg_satisfaction   NUMERIC,
  rating_count       BIGINT,
  completed_count    BIGINT,
  total_count        BIGINT
)
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
AS $$
BEGIN
  IF p_practitioner_id != auth.uid() THEN
    RAISE EXCEPTION 'Access denied';
  END IF;

  RETURN QUERY
  WITH base AS (
    -- All employees with a workout_plan linked to each featured template
    SELECT
      pt.name::TEXT AS plan_name,
      wp.patient_id,
      wp.id         AS workout_plan_id
    FROM plan_templates pt
    JOIN employer_programs ep ON ep.plan_template_id = pt.id
    JOIN patient_links     pl ON pl.practitioner_id  = ep.employer_id
    JOIN workout_plans     wp ON wp.patient_id        = pl.patient_id
                              AND wp.name             = pt.name
                              AND wp.practitioner_id  = ep.employer_id
    WHERE pt.practitioner_id = p_practitioner_id
      AND pt.is_featured      = true
  ),
  totals AS (
    SELECT plan_name, COUNT(DISTINCT patient_id)::BIGINT AS total_count
    FROM base
    GROUP BY plan_name
  ),
  workout_data AS (
    -- Synced workouts matched via employer-assigned planId UUID
    SELECT
      b.plan_name,
      b.patient_id,
      NULLIF(COALESCE(
        NULLIF((sw.data->>'effectivenessRating')::numeric, 0),
        NULLIF((sw.data->>'satisfactionRating')::numeric,  0)
      ), 0) AS eff,
      NULLIF((sw.data->>'enjoymentRating')::numeric, 0)  AS enj
    FROM base b
    JOIN synced_workouts sw ON sw.user_id = b.patient_id
                            AND (sw.data->>'planId') = b.workout_plan_id::text
  ),
  completed AS (
    SELECT plan_name, COUNT(DISTINCT patient_id)::BIGINT AS completed_count
    FROM workout_data
    GROUP BY plan_name
  )
  SELECT
    t.plan_name,
    ROUND(AVG(wd.eff) FILTER (WHERE wd.eff IS NOT NULL), 1) AS avg_effectiveness,
    ROUND(AVG(wd.enj) FILTER (WHERE wd.enj IS NOT NULL), 1) AS avg_enjoyment,
    NULL::NUMERIC                                            AS avg_satisfaction,
    COUNT(*) FILTER (WHERE wd.eff IS NOT NULL OR wd.enj IS NOT NULL) AS rating_count,
    COALESCE(c.completed_count, 0)                          AS completed_count,
    t.total_count
  FROM totals t
  LEFT JOIN workout_data wd ON wd.plan_name = t.plan_name
  LEFT JOIN completed    c  ON  c.plan_name = t.plan_name
  GROUP BY t.plan_name, c.completed_count, t.total_count;
END;
$$;

GRANT EXECUTE ON FUNCTION get_featured_program_ratings(UUID) TO authenticated;


-- ─── 2. New engagement trend RPC ─────────────────────────────────────────────
-- Returns workout count per program week (week 1 = days 0–6 from employer's
-- started_at, week 2 = days 7–13, etc.) across all employers running each template.

CREATE OR REPLACE FUNCTION get_program_engagement_trend(p_practitioner_id UUID)
RETURNS TABLE(
  plan_name     TEXT,
  week_number   INT,
  workout_count BIGINT
)
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
AS $$
BEGIN
  IF p_practitioner_id != auth.uid() THEN
    RAISE EXCEPTION 'Access denied';
  END IF;

  RETURN QUERY
  SELECT
    pt.name::TEXT                                                           AS plan_name,
    (FLOOR((sw.date::date - ep.started_at::date) / 7.0) + 1)::INT         AS week_number,
    COUNT(*)::BIGINT                                                        AS workout_count
  FROM plan_templates pt
  JOIN employer_programs ep ON ep.plan_template_id = pt.id
  JOIN patient_links     pl ON pl.practitioner_id  = ep.employer_id
  JOIN workout_plans     wp ON wp.patient_id        = pl.patient_id
                            AND wp.name             = pt.name
                            AND wp.practitioner_id  = ep.employer_id
  JOIN synced_workouts   sw ON sw.user_id           = wp.patient_id
                            AND (sw.data->>'planId') = wp.id::text
                            AND sw.date::date       >= ep.started_at::date
                            AND sw.date::date       <= ep.ends_at::date
  WHERE pt.practitioner_id = p_practitioner_id
    AND pt.is_featured      = true
  GROUP BY pt.name, week_number
  ORDER BY pt.name, week_number;
END;
$$;

GRANT EXECUTE ON FUNCTION get_program_engagement_trend(UUID) TO authenticated;
